...
import { Plus, Trash2, Save, ArrowLeft, Loader2, ShieldAlert, Search, Filter, Calendar, Edit, Eye } from 'lucide-react';
...
const AddGRNPage = () => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number, field: string) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const inputs = document.querySelectorAll('table input, .grid input, select');
      const currentIndex = Array.from(inputs).indexOf(e.target as HTMLElement);
      
      // අන්තිම Price field එකේදී Enter ගැහුවොත් අලුත් Row එකක් දානවා
      if (field === 'pr') {
        const newItem = { product_id: '', qty_cases: 0, qty_bottles: 0, unit_price: 0, bpc: 12 };
        setItems([...items, newItem]);
        setTimeout(() => {
          const updatedInputs = document.querySelectorAll('table input, .grid input, select');
          (updatedInputs[currentIndex + 1] as HTMLElement)?.focus();
        }, 50);
  ...
  const handleSaveGRN = async () => {
    if (!grnData.supplier_id || items.some(i => !i.product_id)) return alert("කරුණාකර සියලු විස්තර සම්පූර්ණ කරන්න!");
    setLoading(true);
    ...
    try {
      ...
      if (editId) {
        await supabase.from('grn_master').update({
        ...
        const { data: master } = await supabase.from('grn_master').insert([{
          ...grnData, total_amount: totalAmount, company_id: company?.id
        }]).select().single();
        masterId = master.id;
      }

      for (const item of items) {
        const totalBottles = (item.qty_cases * item.bpc) + item.qty_bottles;
        await supabase.from('grn_items').insert([{
          grn_id: masterId,
          product_id: item.product_id,
          quantity: totalBottles,
          unit_price: item.unit_price,
          total_price: (totalBottles / item.bpc) * item.unit_price,
          company_id: company?.id
        }]);
        
        const { data: prod } = await supabase.from('inventory').select('quantity').eq('id', item.product_id).single();
        await supabase.from('inventory').update({ quantity: (prod?.quantity || 0) + totalBottles }).eq('id', item.product_id);
      }
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
         <h1 className="text-2xl font-black italic uppercase">{editId ? 'Edit' : 'New'} GRN <span className="text-blue-600">Entry</span></h1>
         {editId && <button onClick={() => { setEditId(null); setItems([{product_id: '', qty_cases: 0, qty_bottles: 0, unit_price: 0, bpc: 12}]); }} className="text-red-500 font-bold text-xs uppercase">Cancel Edit</button>}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-6 rounded-3xl shadow-sm border mb-6">
          <select 
            ...
            onKeyDown={(e) => handleKeyDown(e, 0, 'supp')}
          >
            <option value="">Select Supplier</option>
            {suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>
          <input type="text" placeholder="Bill No" className="border p-3 rounded-xl font-bold" value={grnData.bill_no} onChange={e => setGrnData({...grnData, bill_no: e.target.value})} onKeyDown={(e) => handleKeyDown(e, 0, 'bill')}/>
          <input type="date" className="border p-3 rounded-xl font-bold" value={grnData.grn_date} onChange={e => setGrnData({...grnData, grn_date: e.target.value})} />
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border overflow-hidden mb-10">
          <table className="w-full">
              <thead className="bg-gray-50 text-[10px] font-black uppercase text-gray-400">
                <tr>
                   <th className="p-4 text-left">Product (Search)</th>
                   <th className="p-4">Qty (Cs/Bt)</th>
                   ...
                   <tr key={index} className="border-t border-gray-50">
                      ...
                      <td className="p-2">
                         <div className="flex gap-1 justify-center">
                            <input type="number" className="w-16 p-2 bg-gray-50 rounded-lg text-center font-bold" value={item.qty_cases} onChange={e => {const n=[...items]; n[index].qty_cases=parseInt(e.target.value)||0; setItems(n);}} onKeyDown={(e)=>handleKeyDown(e, index, 'cs')}/>
                            <input type="number" className="w-16 p-2 bg-blue-50 rounded-lg text-center font-bold" value={item.qty_bottles} onChange={e => {const n=[...items]; n[index].qty_bottles=parseInt(e.target.value)||0; setItems(n);}} onKeyDown={(e)=>handleKeyDown(e, index, 'bt')}/>
                         </div>
                      </td>
                      <td className="p-2">
                         <input type="number" className="w-full p-2 bg-gray-50 rounded-lg text-right font-bold" value={item.unit_price} onChange={e => {const n=[...items]; n[index].unit_price=parseFloat(e.target.value)||0; setItems(n);}} onKeyDown={(e)=>handleKeyDown(e, index, 'pr')}/>
                      </td>
          ...
          <button onClick={() => setItems([...items, {product_id: '', qty_cases: 0, qty_bottles: 0, unit_price: 0, bpc: 12}])} className="w-full p-3 text-[10px] font-black text-blue-500 uppercase">+ Add New Item Manually</button>
      </div>

      <div className="flex justify-between items-center bg-slate-900 p-6 rounded-3xl mb-10">
          <h2 className="text-white font-black">TOTAL: <span className="text-blue-400">LKR {calculateTotal().toLocaleString()}</span></h2>
          <button onClick={handleSaveGRN} className="bg-blue-600 text-white px-8 py-3 rounded-xl font-black uppercase text-xs flex items-center gap-2">
                 ...
                 {grnHistory.map(grn => (
                    <tr key={grn.id} className="hover:bg-gray-50">
                       ...
                       <td className="p-3 flex justify-center gap-2">
                          <button onClick={() => handleEdit(grn)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit size={16}/></button>
                       </td>
                    </tr>
                 ))}
              </tbody>
